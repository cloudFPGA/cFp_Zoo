...
## @file   test_sobel_numpi_video_threaded.py
## @author DID
## @date   October 2021
## @brief  A python script for testing the cF sobel kernel in multi-threaded environment. 

'''
Sobel multithreaded video processing sample.
Usage:
   test_sobel_numpi_video_threaded.py {<video device number>|<video file name>}
...
def main():
    ...
    config_file=os.environ['cFpRootDir'] + "HOST/vision/sobel/languages/cplusplus/include/config.h"
    ...
    def process_frame(frame, t0, threaded_mode, accel_mode, fpga, fpgas):
        ...
        if accel_mode:
            ...
            image = frame.flatten()
            output_array = _trieres_sobel_numpi.sobel(image, total_size, fpga[0], fpga[1])
